...
import {deleteTodoList, addTodo, linkTodo, unLinkTodo, deleteTodos, deleteTodo} from '../actions';
...
let todoList = React.createClass({
  ...
  render() {
        ...
        let todoItems = this.props.todos.map((todo,index) => {
            let todoContent = (<TodoItem item={todo}/>);
            let onDelete = () => {
                console.log("ondelete");
                return this.deleteTodo(todo);
            }
            return (<ListGroupItem key={index}><PresentDelete onDelete={onDelete} content={todoContent}/></ListGroupItem>);
         });
        return (
            <div>
